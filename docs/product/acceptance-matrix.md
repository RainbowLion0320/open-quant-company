...
| 4.9 | Agent paper order preview/proposal/submit gate | `broker/paper_core.py`, `agent_os/runtime.py`, `broker/persistence.py`, `web/frontend/src/views/CEOOffice.vue` | `test_agent_os_contracts.py`, `test_data_layout_contracts.py`, `test_ceo_office_frontend_contracts.py` | `POST /api/agent/paper/proposals`, `POST /api/agent/paper/actions/{id}/submit`, `astroq agent paper propose/submit`, `/` CEO Office action detail | PaperBroker preview 不改现金/持仓/订单；通过 preview 后只创建 `approval_required` 的 `paper_order` action 和 evidence artifact；approved submit 会重新 preview/risk gate，过期或 stale preview 会 blocked，成功后写 `AgentRun`、reconciliation evidence、PaperBroker state/trades/NAV；CEO Office 显示 paper preview/risk 摘要并使用专用 submit path | PARTIAL | 仍需 cancel semantics 和更完整 reconciliation 视图 |
