...
| 7.3 | ApprovalPolicy 审批系统 | `agent_os/approval.py`, `agent_os/runtime.py` | `test_agent_os_contracts.py` | `POST /api/agent/actions/{id}/approve`, `POST /api/agent/actions/{id}/reject`, `astroq agent approve/reject` | write/backfill/backtest/paper/live/code risk level 默认进入 approval-required 状态；未审批 action dispatch 只记录 blocked run，不执行命令 | PARTIAL | 还未接入完整工具执行前审批队列和 expiry/cancel |
