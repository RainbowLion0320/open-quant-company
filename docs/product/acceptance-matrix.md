...
| 7.2 | Agent Runtime 本地会话/消息/行动/运行 ledger | `agent_os/runtime.py`, `agent_os/ledger.py`, `agent_os/schemas.py` | `test_agent_os_contracts.py` | `GET/POST /api/agent/sessions`, `POST /api/agent/sessions/{id}/messages`, `POST /api/agent/actions/{id}/run`, `GET /api/agent/runs/{id}`, `astroq agent sessions/session/message/action show/run` | 创建 session/message/action/run 后可从 CLI/API 读取；固定 registry 命令可 dispatch 并记录 succeeded/failed/blocked run | PARTIAL | 真实 desk workflow dispatch、长任务状态流和安全参数绑定尚未完成 |
...
> **说明：** `功能可验收` 表示该能力有完整可用代码路径。`规划中` 表示已有产品/行为契约但还未实现，不计入当前功能完成数。`质量债条目` 统计"缺口"列非 `—` 的行，包含已实现能力的质量债和规划能力的未实现缺口。当前剩余 10 项缺口：候选策略需要真实 OOS 实证、候选策略证据报告需接入更多真实 baseline 结果、前端 vendor/ECharts/DWP chunk 分包警告需继续优化；Agent Company OS 的 ledger、approval、evidence、desk registry、固定命令 dispatch、固定 registry tool permission enforcement、handoff ledger/resolve 和 CEO Office 已有 foundation，但仍缺 automatic desk workflow dispatch、web-route evidence navigation、streaming、reports 和 live adapter。
