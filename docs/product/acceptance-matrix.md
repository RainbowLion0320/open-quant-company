...
| 7.7 | MiniQMT/QMT live execution adapter | `broker/live/qmt.py`, `agent_os/runtime.py` | `test_agent_os_contracts.py`, `test_ceo_office_frontend_contracts.py` | `GET /api/agent/live/readiness`, `POST /api/agent/live/preview`, `POST /api/agent/live/proposals`, `POST /api/agent/live/actions/{id}/submit`, `GET/POST /api/agent/live/kill-switch(/activate,/deactivate)`, `astroq agent live readiness/preview/propose/submit/kill-switch`, CEO Office live readiness card | live 默认关闭；缺 SDK/权限/account/kill switch 返回 blocked readiness，不 import crash；订单预览统一返回 `approval_required=true`、`submitted=false`、`paper_fallback=false`，并基于 readiness、evidence、现金、单票集中度、总敞口、日订单数、可交易性、数据新鲜度和券商账户一致性给出 risk gate 阻断；live proposal 只在 preview 通过后生成 approval-required action；approved submit 会重新 preview/risk gate、只调用 live adapter、写 `live.live_order.submit` run 和 live reconciliation evidence；本地 kill switch 可激活/解除，激活时取消 queued live_order actions，并在触碰 live adapter 前阻断 preview/propose/submit；默认 MiniQMT adapter 返回 `live_submission_not_integrated`，不伪造实盘订单 | PARTIAL | drawdown/VaR/sector 等更深组合风控、真实 MiniQMT/QMT SDK submit 和 scheduled reconciliation 尚未完成 |
