...
| 7.2 | Agent Runtime 本地会话/消息/行动/运行 ledger | `agent_os/runtime.py`, `agent_os/ledger.py`, `agent_os/schemas.py` | `test_agent_os_contracts.py` | `GET/POST /api/agent/sessions`, `POST /api/agent/sessions/{id}/messages`, `POST /api/agent/actions/{id}/run`, `GET /api/agent/runs/{id}`, `GET/POST /api/agent/memory(/export)`, `astroq agent sessions/session/message/action show/run/memory export` | 创建 session/message/action/run 后可从 CLI/API 读取；固定 registry 命令可 dispatch 并记录 succeeded/failed/blocked run；transparent memory 可 inspect/export 到 `var/artifacts/agent/memory/` | PARTIAL | 真实 desk workflow dispatch、长任务状态流、安全参数绑定和 memory prune/clear 尚未完成 |
