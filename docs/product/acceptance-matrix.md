...
| 4.9 | Agent paper order preview/proposal/submit gate | `broker/paper_core.py`, `agent_os/runtime.py`, `broker/persistence.py` | `test_agent_os_contracts.py`, `test_data_layout_contracts.py` | `POST /api/agent/paper/proposals`, `POST /api/agent/paper/actions/{id}/submit`, `astroq agent paper propose/submit` | PaperBroker preview 不改现金/持仓/订单；通过 preview 后只创建 `approval_required` 的 `paper_order` action 和 evidence artifact；approved submit 会重新 preview/risk gate，过期或 stale preview 会 blocked，成功后写 `AgentRun`、reconciliation evidence、PaperBroker state/trades/NAV | PARTIAL | 仍需 CEO Office 订单卡片 UI、cancel semantics 和更完整 reconciliation 视图 |
