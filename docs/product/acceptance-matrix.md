...
| 7.5 | CEO Office 默认首页 | `web/frontend/src/views/CEOOffice.vue`, `web/frontend/src/router/index.ts`, `web/frontend/src/api/modules/agent.ts` | `test_ceo_office_frontend_contracts.py`, `test_frontend_i18n_contracts.py`, browser smoke | `/` CEO Office, `/market` 市场总览 | 默认首页显示 CEO 对话、desk status、审批队列、handoff 面板、handoff resolve、action detail、run history 和 evidence detail；市场总览迁移到 `/market` | PARTIAL | web-route evidence navigation、streaming、完整报告卡片和 automatic desk response generation 仍未实现 |
...
> **说明：** `功能可验收` 表示该能力有完整可用代码路径。`规划中` 表示已有产品/行为契约但还未实现，不计入当前功能完成数。`质量债条目` 统计"缺口"列非 `—` 的行，包含已实现能力的质量债和规划能力的未实现缺口。当前剩余 10 项缺口：候选策略需要真实 OOS 实证、候选策略证据报告需接入更多真实 baseline 结果、前端 vendor/ECharts/DWP chunk 分包警告需继续优化；Agent Company OS 的 ledger、approval、evidence、desk registry、固定命令 dispatch、固定 registry tool permission enforcement、handoff ledger/resolve、transparent memory inspect/export 和 CEO Office 已有 foundation，但仍缺 automatic desk workflow dispatch、memory prune/clear、web-route evidence navigation、streaming、reports 和 live adapter。
