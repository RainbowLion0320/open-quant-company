...
| 7.5 | CEO Office 默认首页 | `web/frontend/src/views/CEOOffice.vue`, `web/frontend/src/router/index.ts`, `web/frontend/src/api/modules/agent.ts` | `test_ceo_office_frontend_contracts.py`, `test_frontend_i18n_contracts.py`, browser smoke | `/` CEO Office, `/market` 市场总览 | 默认首页显示 CEO 对话、deterministic desk response、session 创建/归档、desk status、审批队列、handoff 面板、handoff resolve、action detail、run history、run artifact evidence links、report cards、evidence detail 和安全 web-route evidence 深链；市场总览迁移到 `/market` | PARTIAL | streaming 和 richer desk reasoning 仍未实现 |
...
| 7.8 | CEO reports and operating rhythm | `agent_os/reports.py`, `agent_os/runtime.py` | `test_agent_os_contracts.py`, `test_ceo_office_frontend_contracts.py` | `GET/POST /api/agent/reports`, `astroq agent reports/report`, CEO Office report cards | Daily CEO brief、weekly review、audit pack、data quality、risk、execution reconciliation、engineering digest、release audit 写入 `var/artifacts/agent/reports/` JSON/Markdown，引用 session/action/run/handoff evidence，缺失 evidence 明确列出，并登记为可解析 `report` evidence | PARTIAL | 定时 operating rhythm、CEO Office 模板选择器和更深入的跨 artifact 报告聚合尚未完成 |
