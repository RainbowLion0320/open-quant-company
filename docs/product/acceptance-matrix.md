...
> **说明：** `功能可验收` 表示该能力有完整可用代码路径。`规划中` 表示已有产品/行为契约但还未实现，不计入当前功能完成数。`质量债条目` 统计"缺口"列非 `—` 的行，包含已实现能力的质量债和规划能力的未实现缺口。当前剩余缺口包括：候选策略需要真实 OOS 实证、候选策略证据报告需接入更多真实 baseline 结果、前端 vendor/ECharts/DWP chunk 分包警告需继续优化；Agent Company OS 的 ledger、approval/action expiry、evidence safe navigation/snapshot、desk registry、deterministic desk workflow、固定命令 dispatch、固定 registry tool permission enforcement、handoff ledger/resolve、transparent memory inspect/export/prune/clear、evidence-cited report artifacts、fixed artifact-context report aggregation、paper order proposal/approved submit/cancel gate、paper reconciliation summary、default-disabled live readiness probe、live order preview/proposal/submit contract、本地 live kill switch、显式/定时 operating-rhythm runner、报告通知触发和 CEO Office 已有 foundation，但仍缺 advanced desk reasoning、streaming、advanced semantic report synthesis 和真实 MiniQMT/QMT SDK submit/reconcile。
