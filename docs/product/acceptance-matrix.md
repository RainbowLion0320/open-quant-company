...
| 7.8 | CEO reports and operating rhythm | `agent_os/reports.py`, `agent_os/runtime.py`, `web/frontend/src/views/CEOOffice.vue` | `test_agent_os_contracts.py`, `test_ceo_office_frontend_contracts.py` | `GET/POST /api/agent/reports`, `POST /api/agent/reports/rhythm`, `astroq agent reports/report/rhythm`, CEO Office report cards | Daily CEO brief、weekly review、audit pack、data quality、risk、execution reconciliation、engineering digest、release audit 写入 `var/artifacts/agent/reports/` JSON/Markdown，引用 session/action/run/handoff evidence，缺失 evidence 明确列出，并登记为可解析 `report` evidence；CEO Office 可选择模板并生成对应报告，也可显式运行 due operating rhythm 并写 rhythm audit artifact | PARTIAL | 后台定时器、通知触发和更深入的跨 artifact 报告聚合尚未完成 |
