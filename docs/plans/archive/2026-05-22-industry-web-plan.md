...
> 归档: 2026-05-23，已由 `docs/development-plan.md` 取代为当前计划入口。
...
- `sector_performance_snapshot`: 行业 1D/5D/20D/60D 动量和数据来源快照。
...
| `sector_performance_snapshot` | computed | daily | 行业 1D/5D/20D/60D 动量和数据来源 |
