...
| 2.15 | Strategy Catalog 策略元数据权威目录 | `research/strategy_catalog.py`, `data/registry.py` | `test_strategy_catalog.py` | `GET /api/strategies/catalog` → `Strategies.vue` | 策略目录含类型、层级、生命周期、数据需求和输出契约 | OK | — |
...
| 5.12 | Monitor/Settings/Config Center 职责边界清晰 | `ActivityMonitor.vue` + `Settings.vue` + `ConfigCenter.vue` | `test_web_system_contracts.py` | `/system` tabs | Monitor 只读运行观测；Settings 管基础设置和审计；Config Center 按 schema 编辑可调参数 | OK | — |
