...
|---|--------------|---------|------|-----------|---------|------|------|
| 1.1 | DataRegistry 声明式维度配置 | `config/settings.yaml` → `data/data_registry.py` | `test_datahub_contracts.py:test_data_registry_contract_is_valid` | `GET /api/system/db-health` → `/datahub?tab=health` | registry validate 无错误，扫描使用注册表路径 | OK | — |
...
| 1.3 | Manifest 元数据记录 (schema/row_count/hash) | `data/datahub.py` | `test_datahub_contracts.py:test_datahub_writes_manifest_for_parquet` | `GET /api/system/db-health` (DB Health 页面) | 检查 `data/store/_manifest/datasets.parquet` 有记录 | OK | — |
...
| 2.15 | Strategy Catalog 策略元数据权威目录 | `research/strategy_catalog.py`, `data/registry.py` | `test_strategy_catalog.py` | `GET /api/strategies/catalog` → `Strategies.vue` | 策略目录含类型、层级、生命周期、config_key、数据需求和输出契约 | OK | — |
| 2.16 | 策略 runtime mode 生产隔离 | `data/strategy_plugins.py`, `scripts/compute_signals.py`, `web/api/jobs.py` | `test_strategy_runtime_gates.py` | `POST /api/strategies/run` (`mode=production/research`) | production 默认排除 candidate；Strategy Lab 候选按钮显式 research scan | OK | — |
...
| 4.7 | Cron 日志可观测 | `data/cron_logger.py` | — | `GET /api/system/cron-jobs` + JSONL 文件 | 每次 cron 执行后日志 JSONL 有记录 | OK | — |
...
| 5.11 | System monitor (CPU/MEM/DISK) | `web/api/routes/system.py` | `test_web_system_contracts.py` | `/system?tab=monitor` → `ActivityMonitor.vue` | 资源面板 + DeepSeek 用量 + Top 进程 + API Health/Cron Jobs | OK | — |
