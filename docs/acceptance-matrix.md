...
|---|--------------|---------|------|-----------|---------|------|------|
| 1.1 | 34 维度注册表声明式配置 | `config/settings.yaml` → `data/data_registry.py` | `test_datahub_contracts.py:test_data_registry_contract_is_valid` | `GET /system/health` → `DatabaseHealth.vue` | `make scan` 成功扫描全量股票 | OK | — |
...
| 1.10 | PIT 特征构建 (月度切片) | `data/feature_store.py` | `test_architecture_contracts.py:test_build_features_import_is_safe` | — | `python scripts/build_features.py` 生成 `data/store/features/YYYY-MM.parquet` | OK | P0-5 将增强 |
...
| 1.13 | DB Health 扫描 | `data/results_db.py` | `test_datahub_contracts.py:test_db_health_scans_moneyflow_symbol_and_tushare_daily` | `GET /system/health` → `DatabaseHealth.vue` | Web 页面 34 维度状态表格 | OK | — |
...
| 2.2 | 多因子四维加权打分 | `signals/multifactor.py` | `test_boundary.py` (动量计算) | `GET /signals/multifactor` → `Signals.vue` | 固定日期+股票池评分结果可复现 | OK | — |
...
| 3.3 | PIT 零前视偏差 | `backtest/run_all_strategies.py` | — | — | 构造未来暴涨样本 → 策略不提前买入 | OK | P0-5 将增强自动化检测 |
...
| # | PRD/Spec 条目 | 代码文件 | 测试 | API / Web | 手工验收 | 状态 | 缺口 |
|---|--------------|---------|------|-----------|---------|------|------|
| 5.1 | Vue 3 SPA + Pinia + ECharts + Tailwind | `web/frontend/` | — | 11 个页面视图 | `npm run build` 通过 | OK | — |
