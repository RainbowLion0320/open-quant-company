...
| 1.10 | PIT 特征构建 (as-of 日期视图) | `data/features/feature_store.py`, `scripts/build_features.py` | `test_architecture_contracts.py:test_build_features_import_is_safe`, `test_asof_pit_feature_view.py` | — | `python scripts/build_features.py --frequency daily` 生成 `var/store/features/YYYY-MM-DD.parquet`；`YYYY-MM.parquet` 月末快照兼容 | OK | — |
