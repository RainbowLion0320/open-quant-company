...
Web 平台提供 Open Quant Company Console — Vue 3 SPA 前端 + FastAPI 后端 + WebSocket 实时推送。当前一级导航收敛为市场总览、市场研究、策略实验室、组合执行、流程图、数据中台、系统控制；原子功能通过二级 tab 或关键参数 Pipeline 访问。

长期 Agent Company OS 目标中，默认 `/` 将从市场总览迁移为 CEO Office 对话主控页，原市场总览迁移到 `/market`。该目标的正式契约见 [07-agent-company-os.md](07-agent-company-os.md)；本 spec 同时记录当前 Web 平台和该长期路由目标。
...
- **证据下钻** — CEO Office 未来只汇总行动和证据，详细数据、策略、生命周期、CodeGraph、AST 和测试设计仍由对应页面承载
...
│   Router: current market / research / strategy-lab      │
│           portfolio / pipeline / datahub / system       │
│   Target: CEO Office (/) + market overview (/market)    │
└──────────────────────┬──────────────────────────────┘
                       │ HTTP REST + WebSocket
┌──────────────────────▼──────────────────────────────┐
│              web/api/ (FastAPI)                        │
...
**Agent Company OS 长期路由目标:**

| 路由 | 页面 | 功能 |
|------|------|------|
| `/` | CEO Office | 默认首页；用户作为 CEO 与 Data / Research / Risk / Execution / Engineering / Reporting desk agents 对话，审批行动，查看证据引用 |
| `/market` | 市场总览 | 承接当前 `/` 市场总览能力：regime、宏观快照、行业脉冲和核心指数相对强弱 |
...
该迁移必须保留可回溯性：CEO Office 只做主控和摘要，不能替代 DataHub、Strategy Lab、Pipeline、System 等专业视图。
...
**规划中的 Agent Company OS API:**

| 模块 | 规划文件 | 规划端点 |
|------|----------|----------|
| Agent Company OS | `routes/agent.py` 或等价 canonical 模块 | `GET /api/agent/sessions`, `POST /api/agent/sessions`, `POST /api/agent/sessions/{session_id}/messages`, `GET /api/agent/actions`, `POST /api/agent/actions/{action_id}/approve`, `POST /api/agent/actions/{action_id}/reject`, `GET /api/agent/evidence/{evidence_id}`, `GET /api/agent/desks` |

这些端点是长期契约，不属于当前实现；实现前不得在 Web 端调用假端点或展示伪成功。
