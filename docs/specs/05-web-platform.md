...
| Agent Company OS | `routes/agent.py` | `GET /api/agent/sessions`, `POST /api/agent/sessions`, `GET /api/agent/sessions/{session_id}`, `POST /api/agent/sessions/{session_id}/messages`, `GET /api/agent/actions`, `GET /api/agent/handoffs`, `POST /api/agent/handoffs/{handoff_id}/resolve`, `GET /api/agent/actions/{action_id}`, `POST /api/agent/actions/{action_id}/approve`, `POST /api/agent/actions/{action_id}/reject`, `POST /api/agent/actions/{action_id}/run`, `GET /api/agent/runs/{run_id}`, `GET /api/agent/evidence/{evidence_id}`, `GET /api/agent/desks` |
...
这些端点当前覆盖 Phase 1 foundation runtime、Phase 2 CEO Office 基础页面和 Phase 3 handoff foundation：本地 ledger、审批状态、证据解析、desk registry、固定 registry 命令的安全/已批准 dispatch、跨 desk handoff ledger、handoff resolve、默认首页、会话消息写入、审批队列、handoff 面板、action detail、run history 和 evidence detail 展示。真实 desk reasoning、广义 tool execution、memory governance、streaming 和 live execution 仍按 [07-agent-company-os.md](07-agent-company-os.md) 分阶段实现；实现前不得在 Web 端调用假端点或展示伪成功。
