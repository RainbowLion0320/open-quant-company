...
| Agent Company OS | `routes/agent.py` | `GET /api/agent/sessions`, `POST /api/agent/sessions`, `GET /api/agent/sessions/{session_id}`, `PATCH /api/agent/sessions/{session_id}`, `POST /api/agent/sessions/{session_id}/messages`, `GET /api/agent/actions`, `GET /api/agent/handoffs`, `POST /api/agent/handoffs/{handoff_id}/resolve`, `GET /api/agent/actions/{action_id}`, `POST /api/agent/actions/{action_id}/approve`, `POST /api/agent/actions/{action_id}/reject`, `POST /api/agent/actions/{action_id}/cancel`, `POST /api/agent/actions/expire`, `POST /api/agent/actions/{action_id}/run`, `GET /api/agent/runs/{run_id}`, `GET /api/agent/evidence/{evidence_id}`, `GET /api/agent/desks`, `GET /api/agent/reports`, `POST /api/agent/reports`, `GET /api/agent/memory`, `POST /api/agent/memory/export`, `POST /api/agent/memory/prune`, `POST /api/agent/memory/clear` |
...
这些端点当前覆盖 Phase 1 foundation runtime、Phase 2 CEO Office 基础页面、Phase 3 handoff foundation 和报告 artifact foundation：本地 ledger、会话 metadata 更新、审批状态、action expiry、证据解析、desk registry、deterministic desk response、固定 registry 命令的安全/已批准 dispatch、跨 desk handoff ledger、handoff resolve、transparent memory inspect/export/prune/clear、evidence-cited CEO reports、默认首页、会话消息写入、审批队列、handoff 面板、action detail、run history、report cards 和 evidence detail 展示。Advanced desk reasoning、广义 tool execution、streaming、完整报告节奏和 live execution 仍按 [07-agent-company-os.md](07-agent-company-os.md) 分阶段实现；实现前不得在 Web 端调用假端点或展示伪成功。
