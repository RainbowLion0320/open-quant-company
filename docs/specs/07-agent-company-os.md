...
This spec defines behavior contracts for the Agent Company OS rollout. Foundation runtime pieces, CEO Office, provider-backed desk routing, provider-backed fixed-tool planning, bounded fixed-command dispatch, run event timelines, session snapshot SSE streaming, active subprocess stdout/stderr chunk event recording, transparent memory governance, evidence-cited report artifacts, fixed cross-artifact report context aggregation, report synthesis, paper order preview/proposal/approved-submit/cancel cards with reconciliation evidence, default-disabled live readiness probing, no-submit live broker environment validation and smoke tests, live order preview risk gating, approval-gated live submit/reconciliation contracts, scheduled live reconciliation scans, cron-callable live monitor evidence ticks, live kill-switch operations, explicit MiniQMT/QMT SDK gateway bridge, desk-declared fixed tool registry coverage, Data Desk repair dry-run plus approval workflow, Research sub-capability handling, Portfolio Desk evidence review, and Engineering Desk work-order triage are implemented. CEO Office ordinary conversation is LLM-first and fail-closed; execution remains bounded to fixed-registry actions, with unknown or state-changing items preserved as blockers or approvals.
...
- LLM-first CEO Office routing, fixed-tool planning, and final response generation through configured OpenAI-compatible providers, with fail-closed blockers instead of local keyword or template fallbacks.
...
| `GET` | `/api/agent/model-runtime` | Read the active LLM runtime provider/model/reasoning profile plus current CEO session context usage. Optional `session_id` scopes the CTX estimate to that session's message ledger. | Implemented |
| `GET/PATCH` | `/api/system/llm-runtime` | Read, update, or reset the local global LLM runtime profile used by controlled LLM use cases. Updates persist under `var/db/` and never rewrite tracked settings files. | Implemented |
...
| `POST` | `/api/agent/sessions/{session_id}/autonomy-step` | Run one bounded autonomy step: add a CEO message, use provider-backed routing/tool planning, dispatch only this step's new `read_only` / `dry_run` fixed-registry actions, and skip approval-required, write, paper, and live actions. | Implemented |
...
| `POST` | `/api/agent/sessions/{session_id}/programs` | Create an autonomy program from provider-backed fixed-tool planning. Safe `read_only` / `dry_run` tool phases are retained; unknown, approval-required, write, code, paper, and live items become blockers or Engineering work orders. | Implemented |
...
| `POST` | `/api/agent/sessions/{session_id}/messages` | Add a CEO message, route it through `agent_routing` when `desk` is omitted, select fixed tools through `agent_tool_planning`, validate tool ids/scopes/parameters against `AgentToolRegistry`, and write the final human-facing answer through `agent_response`. Provider or schema failures create blocker messages instead of local keyword/template fallbacks. | Implemented |
| `POST` | `/api/agent/plans` | Preview the same provider-selected fixed-tool plan without writing the agent ledger. Unknown providers, missing secrets, invalid JSON, unknown tools, invalid parameters, and out-of-scope tools fail closed as blockers. | Implemented |
...
| `astroq agent autonomy step --session SESSION_ID [--desk reporting] [--text ...] --json` | Run one bounded provider-planned fixed-registry autonomy step for the session; only newly proposed `read_only` / `dry_run` actions are dispatched. | Implemented |
| `astroq agent autonomy run --session SESSION_ID [--desk reporting] [--text ...] [--max-steps 2] --json` | Run a bounded multi-step provider-planned autonomy loop; each step is filtered through the same fixed-registry safety boundary and stops when no safe action remains, a step is not ready, or the max step cap is reached. | Implemented |
| `astroq agent programs [--session SESSION_ID] --json` | List persisted autonomy programs. | Implemented |
| `astroq agent program create --session SESSION_ID --goal ... [--desk reporting] [--max-steps 6] --json` | Persist a multi-stage autonomy program from provider-backed fixed-tool planning. | Implemented |
...
| `astroq agent message --session SESSION_ID --desk DESK --text TEXT --json` | Add a CEO message and return a provider-backed desk response after fixed-tool planning and validation. | Implemented |
| `astroq agent plan --desk DESK --text TEXT --json` | Preview a side-effect-free provider-backed fixed-tool workflow before creating messages, evidence, actions, handoffs, or work orders. | Implemented |
...
CEO message routing is LLM-first and fail-closed. Web messages that omit `desk`
must call the active LLM runtime profile through `llm.use_cases.agent_routing`
provider. Router output must be JSON with `primary_desk`, `supporting_desks`,
`intent`, `confidence`, `reason`, and `needs_tool`; invalid output creates a
blocked response and never falls back to local keyword routing.

Tool selection is also LLM-first but fixed-registry bounded. The
`llm.use_cases.agent_tool_planning` may only choose tool ids already
declared in `AgentToolRegistry`; the backend validates desk scope, risk level,
and parameter patterns before any action appears. Unknown tools, out-of-scope
tools, invalid parameters, missing provider secrets, timeouts, or invalid JSON
become blockers.

Desk responses separate machine planning from human-facing language. Normal
business wording is generated by `llm.use_cases.agent_response` from CEO text,
router output, accepted tool calls, evidence/action refs, blockers, and tool
results. If that provider is missing, disabled, lacks a secret, or fails, the
response must be an explicit model-unavailable error instead of a local business
template.
Missing artifacts must be reported as missing with a recommended `astroq`
command, never filled with invented values. Tool execution remains explicit
through desk-scoped fixed-registry actions.
...
```json
{
  ...
  "reasoning": [
    {"kind": "agent_router", "primary_desk": "research"},
    {"kind": "agent_tool_planner", "accepted_tool_ids": ["astroq.strategy.compete"]},
    {"kind": "safety", "approval_required_count": 0},
    {"kind": "agent_response", "provider": "mimo"}
  ]
}
```
...
builds a context pack for provider calls: recent messages remain raw, older
...
provider calls return a blocker instead of calling the model.
...
- CEO ordinary conversation uses provider-backed routing, provider-backed fixed-tool planning, and provider-backed final response synthesis. The runtime no longer routes by local keyword profiles or returns local business templates. Router output is limited to known desks; tool planning output is limited to fixed registry tools in the selected desk scope; final wording is generated by `agent_response` from CEO text, accepted tool results, evidence/action refs, and blockers. `agent_routing`, `agent_tool_planning`, `agent_response`, `agent_planning`, and `factor_hypothesis` share one local LLM runtime profile for provider/model/reasoning, while preserving their own timeout/temperature/JSON request shape. Data, Research, Portfolio, Risk, Execution, Engineering, and Reporting desk boundaries remain explicit through `allowed_tools`, `forbidden_actions`, and approval policy. Research technical, sentiment, fundamental, factor, and ML analysis remain sub-capabilities under Research Desk rather than separate top-level desks.
