...
| `POST` | `/api/agent/sessions/{session_id}/run-readonly` | Dispatch proposed read-only actions for one session and skip write/trading actions. | Implemented |
...
| `astroq agent session run-readonly SESSION_ID --json` | Dispatch proposed read-only actions for one session and skip write/trading actions. | Implemented |
...
- Deterministic desk workflows route common CEO intents to specific safe tools, including data source registry diff, 12-strategy competition evidence, backtest dry-run, test design intelligence, and documentation hygiene checks. Reporting daily-brief intent now performs a bounded cross-desk orchestration step by proposing separate Data, Research, and Risk read-only actions with distinct evidence references, and session-level read-only workflow runners can dispatch those proposed safe actions while skipping write/trading actions.
...
- CEO Office is implemented as the default `/` route with session creation, target desk selection for CEO messages, message entry, session read-only workflow runner, desk status, desk drill-down for mandates/tools/evidence requirements/related work, explicit approval policy display, approval queue display, action detail, and run event timeline display; `/market` carries the market overview.
- Actual advanced desk reasoning, broad cross-tool workflow orchestration beyond the deterministic daily-brief path, realtime Web streaming, advanced semantic report synthesis, and real MiniQMT/QMT SDK submission/reconciliation are not yet implemented.
