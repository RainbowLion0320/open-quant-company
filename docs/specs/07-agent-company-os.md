...
- An opt-in semantic planner adapter can draft ambiguous CEO plans, and API/CLI callers can pass an explicit semantic draft object/file for server-side filtering. Draft output is treated as untrusted input, filtered to known `read_only` / `dry_run` fixed-registry tools, scoped to allowed desks, and marked for manual review; autonomous unbounded desk reasoning beyond fixed-registry safety boundaries is not yet implemented.
