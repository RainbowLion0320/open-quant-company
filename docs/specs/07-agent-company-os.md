...
| `POST` | `/api/agent/plans` | Preview a deterministic desk workflow plan, action status previews, approvals, handoffs, and work orders without writing the ledger. | Implemented |
...
- Actual advanced desk reasoning, adaptive cross-tool workflow planning beyond deterministic intent rules, advanced causal-chain report synthesis, and real MiniQMT/QMT SDK submission/reconciliation are not yet implemented.
