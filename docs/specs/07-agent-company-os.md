...
- Rich adaptive desk reasoning beyond deterministic reasoning rows, open-ended adaptive cross-tool workflow planning beyond bounded deterministic/artifact-aware/session-backlog modes, and real-account xtquant gateway smoke tests plus project-ledger cash/position/order reconciliation are not yet implemented.
