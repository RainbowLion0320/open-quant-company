...
- CEO Office is implemented as the default `/` route with session creation, target desk selection for CEO messages, message entry, desk status, desk drill-down for mandates/tools/evidence requirements/related work, explicit approval policy display, approval queue display, action detail, run event timeline display, report cards, live readiness/kill-switch controls, and evidence detail. It intentionally does not expose autonomy step/run/program controls or semantic-planner debug inputs; those remain CLI/API/internal runtime mechanisms. `/market` carries the market overview.
