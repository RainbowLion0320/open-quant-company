...
- Provider-semantic planning remains an explicit API/CLI mode. It is not shown as a CEO Office Web control, and callers must understand that it can call an external LLM provider and write returned token/cost usage to the local generic LLM usage ledger when available.
...
| `GET` | `/api/agent/programs` | List autonomy programs, optionally scoped to a session. Programs persist multi-stage CEO goals, phases, blockers, and fixed-registry execution boundaries. | Implemented |
| `POST` | `/api/agent/sessions/{session_id}/programs` | Create an autonomy program from a deterministic or explicitly semantic-assisted plan. Safe `read_only` / `dry_run` tool phases are retained; unknown, approval-required, write, code, paper, and live items become blockers or Engineering work orders. | Implemented |
| `GET` | `/api/agent/programs/{program_id}` | Read one autonomy program with phases, blockers, current step, and boundary metadata. | Implemented |
| `POST` | `/api/agent/programs/{program_id}/run` | Run pending safe phases in an autonomy program, or dry-run the pending phase list. Dispatch remains fixed-registry only. | Implemented |
| `POST` | `/api/agent/sessions/{session_id}/messages` | Add a CEO message, create a deterministic or explicitly semantic-assisted desk response, and link evidence/actions/handoffs. Payloads may explicitly set `planner_mode=semantic_draft` with a `semantic_draft` object, or `planner_mode=provider_semantic` to ask the configured env-only LLM provider for a draft. Optional `planner_provider` / `planner_model` override the configured `agent_planning` use case for that request. All semantic input is untrusted; unknown providers, disabled providers, missing provider secrets, missing base URLs, missing models, and unsupported protocols fail closed before transport; only safe filtered fixed-registry actions are persisted, and semantic-assisted submissions write a sanitized `Semantic planner audit` artifact evidence under `var/artifacts/agent/semantic_plans/`. | Implemented |
| `POST` | `/api/agent/plans` | Preview a deterministic or explicitly semantic-assisted desk workflow plan, action status previews, approvals, handoffs, reasoning rows, and work orders without writing the agent ledger. Payloads may explicitly set `planner_mode=semantic_draft` or `planner_mode=provider_semantic`; optional `planner_provider` / `planner_model` are request-scoped overrides. Previews write no agent ledger/evidence artifacts, fail closed when the provider is unknown, disabled, missing credentials, missing base URL/model, or unsupported, and safe-filter all proposed actions. If provider semantic mode makes a real LLM call and the provider returns usage, the generic LLM usage ledger records `source=agent_planning`; missing provider/model pricing marks the usage as unpriced instead of estimating fake cost. | Implemented |
...
| `astroq agent autonomy step --session SESSION_ID [--desk reporting] [--text ...] [--semantic-draft-file draft.json \| --provider-semantic] --json` | Run one bounded deterministic or semantic-assisted fixed-registry autonomy step for the session; only newly proposed `read_only` / `dry_run` actions are dispatched. | Implemented |
| `astroq agent autonomy run --session SESSION_ID [--desk reporting] [--text ...] [--max-steps 2] [--semantic-draft-file draft.json \| --provider-semantic] --json` | Run a bounded multi-step deterministic or semantic-assisted autonomy loop; each step is filtered through the same fixed-registry safety boundary and stops when no safe action remains, a step is not ready, or the max step cap is reached. | Implemented |
| `astroq agent programs [--session SESSION_ID] --json` | List persisted autonomy programs. | Implemented |
| `astroq agent program create --session SESSION_ID --goal ... [--desk reporting] [--max-steps 6] [--semantic-draft-file draft.json \| --provider-semantic] --json` | Persist a multi-stage autonomy program from a deterministic or semantic-assisted goal. | Implemented |
| `astroq agent program show PROGRAM_ID --json` | Inspect one autonomy program. | Implemented |
| `astroq agent program run PROGRAM_ID [--dry-run] --json` | Run or preview pending safe phases in one autonomy program. | Implemented |
| `astroq agent message --session SESSION_ID --desk DESK --text TEXT [--semantic-draft-file draft.json \| --provider-semantic] --json` | Add a CEO message and return the deterministic or explicitly semantic-assisted desk response. Semantic draft files and provider drafts are untrusted input and are filtered to safe fixed-registry actions before ledger writes; provider mode uses env-only credentials and fails closed when the secret is missing. | Implemented |
| `astroq agent plan --desk DESK --text TEXT [--semantic-draft-file draft.json \| --provider-semantic] --json` | Preview a side-effect-free deterministic or explicitly semantic-assisted workflow before creating messages, evidence, actions, handoffs, or work orders. Provider semantic planning is explicit opt-in and never writes ledger rows during preview. | Implemented |
...
```json
{
  ...
  "desk_response": {
    ...
    "answer": "<agent_response provider generated answer, or a model-unavailable error>",
    "evidence_refs": ["ev_..."],
...
Desk responses separate machine planning from human-facing language. The
runtime may use deterministic routing, fixed tool selection, local fact packs,
and evidence/blocker validation, but normal business wording is generated by
`llm.use_cases.agent_response`. If that provider is missing, disabled, lacks a
secret, or fails, the response must be an explicit model-unavailable error
instead of a deterministic business-template fallback.
...
- include a deterministic `domain_scorecard` section that maps evidence-backed root causes to Data, Research, Portfolio, Risk, Execution, Engineering, and Reporting desk status plus recommended `astroq` commands;
...
The CEO Office should surface exceptions through messages, action cards, and evidence links. It must deep-link into specialized views rather than duplicating runtime, engineering, reporting, or live-execution control panels.
...
- Foundation runtime is implemented in `agent_os/` for the current bounded Agent Company OS scope.
...
- `astroq agent sessions/session create/session show/session update/autonomy step/autonomy run/programs/program create/program show/program run/message/actions/handoffs/handoff resolve/work-orders/work-order create/work-order update/action show/run/approve/reject/cancel/expire/reports/report/notify report/rhythm (--session or --all-active)/paper propose/paper submit/paper cancel/live readiness/live environment/live smoke/live preview/live propose/live submit/live reconcile/live monitor/live kill-switch status/live kill-switch activate/live kill-switch deactivate/evidence/desks/policies/memory show/memory export/memory prune/memory clear --json` is available.
- `/api/agent/sessions`, `/api/agent/sessions/{session_id}` `GET/PATCH`, `/api/agent/sessions/{session_id}/stream`, `/api/agent/sessions/{session_id}/autonomy-step`, `/api/agent/sessions/{session_id}/autonomy-run`, `/api/agent/programs`, `/api/agent/sessions/{session_id}/programs`, `/api/agent/programs/{program_id}`, `/api/agent/programs/{program_id}/run`, `/api/agent/actions`, `/api/agent/actions/expire`, `/api/agent/handoffs`, `/api/agent/handoffs/{handoff_id}/resolve`, `/api/agent/work-orders`, `/api/agent/work-orders/{work_order_id}`, `/api/agent/actions/{action_id}/run`, `/api/agent/actions/{action_id}/cancel`, `/api/agent/runs/{run_id}`, `/api/agent/runs/{run_id}/stream`, `/api/agent/evidence/{evidence_id}`, `/api/agent/desks`, `/api/agent/policies`, `/api/agent/paper/proposals`, `/api/agent/paper/actions/{action_id}/submit`, `/api/agent/paper/actions/{action_id}/cancel`, `/api/agent/live/readiness`, `/api/agent/live/environment`, `/api/agent/live/smoke`, `/api/agent/live/preview`, `/api/agent/live/proposals`, `/api/agent/live/actions/{action_id}/submit`, `/api/agent/live/reconciliation`, `/api/agent/live/monitor`, `/api/agent/live/kill-switch`, `/api/agent/live/kill-switch/activate`, `/api/agent/live/kill-switch/deactivate`, `/api/agent/reports`, `/api/agent/reports/{report_id}/notify`, `/api/agent/reports/rhythm`, `/api/agent/reports/rhythm/scheduled`, `/api/agent/memory`, `/api/agent/memory/export`, `/api/agent/memory/prune`, and `/api/agent/memory/clear` are available.
- Action dispatch is intentionally bounded to fixed `AgentToolRegistry` command arrays. Read-only actions can run; approval-required actions are blocked until approved; approved fixed-registry templated commands bind only tool-declared safe parameters. Bounded autonomy steps and capped autonomy runs create normal CEO messages and desk responses, optionally use semantic draft/provider planning as untrusted input, then dispatch only newly proposed `read_only` / `dry_run` fixed-registry actions while leaving write, report artifact, paper, live, and code actions queued or skipped for approval. Autonomy programs persist open-ended CEO goals as durable multi-stage phase ledgers; only safe fixed-registry phases can run, and unknown or state-changing items remain blockers, approvals, or work orders.
...
- The fixed registry covers all desk-declared tools, including lifecycle checks, data status, data source capability listing and registry diff, data repair dry-run/approved repair, strategy catalog, strategy competition evidence, backtest dry-run, execution dry-run, MiniQMT/QMT live readiness, AST diagnostics, test design intelligence, docs check, and report generation.
- Deterministic desk workflows route common CEO intents to specific safe tools, including data source registry diff, Data Desk repair dry-run plus approval-required write action, 12-strategy competition evidence, Research strategy-blocker diagnosis across DataHub health and lifecycle gates, Portfolio Desk decision review across strategy evidence and risk/execution readiness, Execution Desk MiniQMT/QMT live-readiness review, Engineering Desk code/bug requests into work orders plus AST/test-design diagnostics, backtest dry-run, test design intelligence, and documentation hygiene checks. Research technical, sentiment, fundamental, factor, and ML analysis remain sub-capabilities under Research Desk rather than separate top-level desks. Data Desk source-capability and repair-planning requests include bounded local evidence summaries while still separating dry-run from approval-required write repair. Risk Desk lifecycle readiness questions include bounded local lifecycle/data evidence summaries while still using read-only checks. Execution Desk readiness/dry-run/live-readiness questions include bounded lifecycle/data evidence summaries while never submitting paper or live orders. Engineering Desk diagnostic requests include bounded AST/test-design evidence summaries while still creating diagnostics/work orders instead of editing the repository. Reporting daily-brief intent performs a bounded cross-desk orchestration step by proposing separate Data, Research, and Risk read-only actions with distinct evidence references. Reporting portfolio-review intent proposes Research strategy competition evidence, Portfolio decision evidence, Risk lifecycle readiness, and Execution dry-run actions. Reporting mixed CEO review requests can produce deterministic `dynamic_multi_intent` plans that combine Data source diff, Research strategy competition, Portfolio decision review, Risk lifecycle, Execution dry-run, and Engineering test/AST diagnostics while preserving fixed tool ids and handoffs. Broad CEO priority requests can produce `artifact_aware` plans from fixed local artifact context, mapping root causes such as data source gaps, lifecycle blockers, strategy evidence blockers, AST risks, and test design risks into owner-desk safe actions. CEO follow-up requests such as "next step" or "continue" can produce `adaptive_session` plans from current session backlog, turning approval-required actions, open handoffs, and open Engineering work orders into safe re-check actions and owner-desk handoffs without auto-approving writes or trades. Requests that combine follow-up language with current evidence/priority language can produce `adaptive_artifact` plans that fuse session backlog with local artifact root causes, dedupe overlapping safe actions, and expose both `session_backlog` and `artifact_context` reasoning. Data source/repair, Research strategy-blocker, Portfolio decision review, Risk lifecycle, Execution readiness, Engineering diagnostics, Reporting daily brief, portfolio review, `dynamic_multi_intent`, `artifact_aware`, `adaptive_artifact`, and `open_ended_adaptive` desk answers plus `artifact_context` reasoning include bounded evidence summaries such as blocker dimensions/reasons, unmapped capability counts, blocked strategy ratios, and AST/test-design risk counts instead of returning a generic template. Open-ended company-wide CEO operating requests can produce `open_ended_adaptive` diagnostic plans across Data, Research, Portfolio, Risk, Execution, and Engineering fixed-registry tools; these plans only propose read-only/dry-run actions and report `open_ended_plan_is_diagnostic_only` as a blocker for direct writes or trades. Workflow previews and desk responses include deterministic reasoning rows for intent match, tool plan, safety, evidence plan, artifact context, session backlog, context fusion, open goal decomposition, and current session context.
