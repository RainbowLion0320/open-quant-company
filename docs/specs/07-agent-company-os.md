...
- An opt-in semantic planner adapter can draft ambiguous CEO plans, and API/CLI callers can pass an explicit semantic draft object/file for server-side filtering in plan preview, message submission, and bounded autonomy steps. Draft output is treated as untrusted input, filtered to known `read_only` / `dry_run` fixed-registry tools, scoped to allowed desks, and marked for manual review; bounded autonomy is implemented inside fixed-registry safety boundaries, while autonomous unbounded desk reasoning beyond those boundaries is not yet implemented.
