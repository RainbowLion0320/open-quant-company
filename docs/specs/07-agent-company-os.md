...
| `GET` | `/api/agent/model-runtime` | Read the configured `agent_response` provider/model, reasoning level, temperature, max context setting, and an estimated current session context usage. Optional `session_id` scopes the usage estimate to that session's message ledger. | Implemented |
...
must call the configured `llm.use_cases.agent_routing` OpenAI-compatible
...
`llm.use_cases.agent_tool_planning` provider may only choose tool ids already
