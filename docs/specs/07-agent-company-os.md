...
| `POST` | `/api/agent/plans` | Preview a deterministic or explicitly semantic-assisted desk workflow plan, action status previews, approvals, handoffs, reasoning rows, and work orders without writing the ledger. Payloads may explicitly set `planner_mode=semantic_draft` or `planner_mode=provider_semantic`; optional `planner_provider` / `planner_model` are request-scoped overrides. Previews remain side-effect-free, write no ledger/evidence artifacts, fail closed when provider credentials are missing, and safe-filter all proposed actions. | Implemented |
