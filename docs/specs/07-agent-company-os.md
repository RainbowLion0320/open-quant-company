...
| `POST` | `/api/agent/sessions/{session_id}/messages` | Add a CEO message, create a deterministic desk response, and link evidence/actions/handoffs. Payloads may explicitly set `planner_mode=semantic_draft` with a `semantic_draft` object; the runtime treats it as untrusted input and persists only safe filtered fixed-registry actions. | Implemented |
| `POST` | `/api/agent/plans` | Preview a deterministic desk workflow plan, action status previews, approvals, handoffs, reasoning rows, and work orders without writing the ledger. Payloads may explicitly set `planner_mode=semantic_draft` with a `semantic_draft` object; previews remain side-effect-free and safe-filtered. | Implemented |
...
| `astroq agent message --session SESSION_ID --desk DESK --text TEXT --semantic-draft-file draft.json --json` | Add a CEO message and return the deterministic or explicitly draft-assisted desk response. Semantic draft files are untrusted input and are filtered to safe fixed-registry actions before ledger writes. | Implemented |
| `astroq agent plan --desk DESK --text TEXT --semantic-draft-file draft.json --json` | Preview a side-effect-free deterministic or explicitly draft-assisted workflow before creating messages, evidence, actions, handoffs, or work orders. | Implemented |
