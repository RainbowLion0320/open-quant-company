...
初始 source 集合固定为 `akshare`、`tushare`、`tencent_finance`、`eastmoney`、`sina_finance`、`tonghuashun`、`exchange_official`、`cninfo`、`computed`。腾讯财经、东方财富、新浪财经、同花顺在 v1 中作为 AKShare backend 或 direct candidate source 治理；只有代码中实际接入的接口才标记为 `project_integrated` 或 `backend_source`，不承诺全量接口稳定可用。腾讯财经当前只记录 `qt.gtimg.cn/q={symbol}` 实时行情文本接口和 `web.ifzq.gtimg.cn/appstock/app/fqkline/get` 日 K 线 JSON 接口为 `candidate/manual_review`，不得自动进入生产 `data_registry`。
