...
初始 source 集合固定为 `akshare`、`tushare`、`tencent_finance`、`eastmoney`、`sina_finance`、`tonghuashun`、`exchange_official`、`cninfo`、`computed`。能力状态分为 `discovered`、`sample_probed`、`contracted`、`project_integrated` 四层；发现接口不等于下载数据，不等于字段契约稳定，也不等于生产接入。腾讯财经、东方财富、新浪财经、同花顺通过 AKShare backend callable 映射和静态 endpoint catalog 进入能力目录，只有代码中实际接入并有数据维度映射的接口才标记为 `project_integrated`。
