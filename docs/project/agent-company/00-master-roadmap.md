...
- Bounded autonomy steps can create a normal CEO message, generate a deterministic desk plan, dispatch only this step's newly proposed `read_only` / `dry_run` fixed-registry actions, and skip approval-required write, report artifact, paper, live, and code actions for CEO review.
