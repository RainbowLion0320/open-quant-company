# Quant Agent — 产品需求文档 (PRD)

> 版本: 1.0 | 日期: 2026-05-21 | 状态: 与当前 Phase 4.x 实现对齐

## 1. 产品定义

Quant Agent 是一个个人A股量化交易系统。分阶段从数据分析助手演进为信号生成系统，最终支持半自动交易。核心理念：**巴菲特价值投资为决策约束层，钱学森控制论为运行机制层**——两者正交，前者管边界和原则，后者管执行和反馈。

- **频率**: 日线级别，低频交易
- **市场**: A股全量（5517只）+ 申万31行业
...
### 3.1 数据管道
从 AKShare + Tushare 拉取多源数据，经清洗、缓存、存储为本地 Parquet，通过 DataHub 统一路径访问。覆盖日线行情、财务摘要、每日估值、资金流向、宏观指标。

### 3.2 信号体系
四种策略生成交易信号：巴菲特价值过滤（能力圈→护城河→安全边际）、多因子打分（四维加权+横截面排名）、LightGBM ML（PIT特征→模型预测）、控制论自适应（regime 检测→参数调整）。因子 DSL 支持声明式因子表达和 LLM 公式解析。

### 3.3 回测引擎
日频回测，严格 PIT 无前视防护。支持 N 策略同时跑对比（锦标赛模式），输出 15 项风险指标（Sharpe/Sortino/Calmar/Alpha/Beta/MaxDD 等）。

### 3.4 执行层
PaperBroker 本地模拟撮合，5 规则风控（仓位上限/总敞口/下单次数/回撤熔断/单笔金额），Parquet 持久化状态和 NAV。Cron 日频调度（15:30 扫描，09:30 执行）。

### 3.5 Web 平台
Vue 3 Quantum Terminal（Pinia + ECharts + Tailwind），FastAPI 后端 9 路由模块，WebSocket 实时进度推送，DuckDB :memory: 零锁查询。
...
- 自动轮转：每文件最多 500 行
- Web 端 DB Health 页面展示 34 维度注册表状态
- 错误通知：支持 Telegram/企业微信/飞书推送（需配置）

### 6.4 性能
- 5517 只全量日线缓存命中时 < 1 秒返回
...
│   ├── data_registry.py          # 34 维度注册表+健康检查
│   ├── cleaner.py                # 6 规则数据清洗
│   ├── symbols.py                # 5517 全A股+申万行业
│   └── cron_logger.py            # Cron 错误可观测性
├── signals/                      # 信号层
│   ├── expression.py             # 因子 DSL 表达式引擎
│   ├── dsl_parser.py             # LLM 公式→计算
│   ├── buffett.py                # 巴菲特三重过滤
│   ├── multifactor.py            # 多因子四维打分
...
├── web/                          # Web 平台
│   ├── api/                      # FastAPI (9 路由 + WebSocket + Jobs)
│   └── frontend/                 # Vue 3 SPA Quantum Terminal
├── models/                       # ML 模型注册表
├── scripts/                      # Cron/工作流脚本
├── tests/                        # 合约测试+边界测试
├── wiki/                         # LLM Wiki 知识库 (18 页)
├── docs/                         # 设计文档 ← 当前目录
│   ├── PRD.md
│   └── specs/
├── Makefile                      # 构建/扫描命令
...
| 阶段 | 能力 | 状态 |
|------|------|------|
| Phase 1 | 数据基础：AKShare 日线+财务，Parquet 存储，DataHub | ✅ 完成 |
| Phase 2 | 信号生成：巴菲特过滤+多因子打分+因子DSL | ✅ 完成 |
| Phase 3 | 回测引擎：日频回测+风险指标+锦标赛 | ✅ 完成 |
| Phase 4.0 | ML 信号：LightGBM + PIT 特征 + LLM 因子发现 | ✅ 完成 |
| Phase 4.1 | 多资产：AssetAdapter + AssetRegistry + Allocator | ✅ 完成 |
| Phase 4.2 | 模拟交易：PaperBroker + Parquet 持久化 + NAV 记录 | ✅ 完成 |
| Phase 4.3 | 风控：5 规则 RiskManager + 熔断机制 | ✅ 完成 |
| Phase 4.4 | Web 平台：Vue 3 Quantum Terminal + FastAPI + WebSocket | ✅ 完成 |
| Phase 5 | 半自动实盘：MiniQMT 对接 + 人工确认 | 🔮 规划中 |
