...
Open Quant Company 不是一个单点策略脚本，也不是远端托管交易平台。它更像一套本地优先的量化公司操作系统：人负责方向和判断，agent 负责把数据、研究、组合、风控、执行、工程和报告工作标准化执行；Web UI 给人看清细节，`astroq` CLI 给 agent、cron 和脚本稳定调用。
...
| Research Desk | 技术面、情绪面、基本面、因子和 ML 研究能力，Strategy Catalog、OOS/IC/ICIR、策略竞赛和候选策略治理 |
| Portfolio Desk | 消费研究证据和风险约束，审查组合权重、仓位、调仓节奏和策略组合优先级 |
| Risk Desk | market regime、风险预算、仓位约束、回撤熔断和执行前门禁 |
| Execution Desk | PaperBroker / MiniQMT-QMT readiness、订单预览、执行演练、对账和 kill switch |
