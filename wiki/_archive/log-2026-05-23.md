...
This historical append-only log is preserved for context. Current implementation history is tracked by git log.
...
- 项目标题: "星盘 — 个人量化研究与执行操作系统 — 日频 · 自托管 · AI增强"
...
## [2026-05-15] redesign | 星盘终端 Web UI v4.0
...
- **设计系统**: 新建 `src/assets/astrolabe-quant.css` — 完整 token 体系，统一全站
...
### Codex 改动范围
11 文件, 972 insertions: ActivityMonitor.vue(573行), astrolabe-quant.css(67行), Settings.vue(188行), Market.vue(60行), HindsightGraph.vue(114行), Backtest/Portfolio/StockDetail/Strategies/Stocks/Signals
...
Codex: 提取为 `astrolabe-quant.css` 的 `.view-page`, `.card-pad`, `.section-heading`, `.metric-card`
...
- 页面容器用 `.view-page` class (astrolabe-quant.css 全局共享)
