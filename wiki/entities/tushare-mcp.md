...
- URL: `https://api.tushare.pro/mcp/?token=***
- MCP配置: `~/.hermes/config.yaml` → `mcp_servers.tushare`
- Token: 环境变量 `TUSHARE_TOKEN` 优先, `config/settings.yaml` 兜底 → 统一由 `data/ingestion/tushare_utils.py::get_tushare_token()` 获取
- Token 已从 settings.yaml 剥离，改为环境变量管理（安全性）
...
- 数据源分工: [[data-sources]]
- 财务因子提取: [[financial-cache]]
- 系统数据层: [[system-architecture]]
