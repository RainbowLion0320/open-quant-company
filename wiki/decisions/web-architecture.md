...
| 系统控制 | `/system` | 二级 tab: 系统信息 + 系统设置 + 配置中心 + Hindsight 记忆图谱 |
...
- 新增: `/api/system/api-health` (AKShare/Tushare/LLM providers/Hindsight/Telegram 配置状态)
- 新增: `/api/system/cron-jobs` (cron job 状态, 读 ~/.hermes/cron/jobs.json)
- 新增: `/api/system/llm-usage` (provider 余额 + 本项目 LLM API response usage ledger; `/deepseek-usage` 保留兼容)
- 前端: ActivityMonitor.vue, CPU/MEM/DISK + LLM provider 项目用量 + Top 进程 + API Health + Cron Jobs
- LLM 用量来源: provider 余额 API + 本项目 API response usage ledger，不依赖 Chrome CDP/Cookie。
...
### 记忆图谱 (2026-05-17, 2026-06-02 对齐)

HindsightGraph 使用 Three.js WebGL 3D 星空图，零外部图谱库依赖。直接从 `GET /api/hindsight/graph` 拉取节点和链接数据，渲染为交互式知识图谱。节点按类型分色（青色 observation / 紫色 experience / 金色 world），链接按来源区分（entity共享/同轮对话/合并提炼/tag共享）。悬浮显示 tooltip，点击展开详情面板并高亮关联边；旋转、缩放、平移全支持。

物理模拟：节点间 3D 斥力（inverse-square）+ 图中心引力 + 链接弹簧力 + 阻尼。Three.js 负责 WebGL 渲染和 OrbitControls 交互。数据非实时——页面加载时拉取一次，手动按钮可刷新。
