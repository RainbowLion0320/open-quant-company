...
├── log.md              # 变更日志索引; 详细历史以 git log 为准
├── raw/                # Layer 1: 不可变原始素材 (可选目录)
...
├── reference/          # Layer 2: 参考文档 (数据维度、命令速查)
├── queries/            # Layer 2: 归档的问答结果 (可选目录)
└── _archive/           # 已取代的页面存档
```
...
- 不再把所有操作追加到 `log.md`。实现历史以 git log 为准；`log.md` 只保留少量无法从 git 恢复的人工说明。
...
- Wiki 存长期知识、概念和设计理由。
- `docs/plans/archive/` 存已完成或废弃的执行计划。
...
created: YYYY-MM-DD
updated: YYYY-MM-DD
type: concept | entity | comparison | decision | query | reference
tags: [from taxonomy]
