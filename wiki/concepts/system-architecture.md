...
│  │ 同花顺财务│  │ PE/PB/估值   │  │ signals/features/  │  │
│  └──────────┘  └──────────────┘  └───────────────────┘  │
│  DuckDB :memory: → CREATE VIEW FROM read_parquet()       │
└──────────────────────────────────────────────────────────┘
```
...
| 基本面 | 8 | 同花顺财报 | fund_roe, fund_gross_margin, fund_de_ratio, fund_roe_5y_avg, fund_gm_trend |
| 估值 | 6 | Tushare daily_basic | val_pe, val_pb, val_ps, val_pe_percentile, val_total_mv |
| **LLM发现** | **7** | **deepseek-v4-pro** ★ | vol_adj_mom_5d, midpoint_bias, intraday_close_strength, volume_conviction, open_gap_ma20, upside_intraday_range, volume_vol_ratio |
