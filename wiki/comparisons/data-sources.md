...
过去只写“AKShare vs Tushare”会漏掉两个问题：一是 AKShare 本身包装了腾讯、东方财富、新浪、同花顺等 backend；二是外部源能提供的接口远多于项目当前接入的维度。现在用 Source Capability Registry 单独治理外部能力，再和 `data_registry` 做 diff。
...
```bash
astroq data sources audit --source all --json
astroq data sources diff-registry --json
```
