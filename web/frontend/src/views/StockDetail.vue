<template>
  <div class="view-page stock-detail-page">
    <div class="page-header">
      <div>
        <h1 class="page-title">{{ t('stockDetail.title') }}</h1>
        <p class="page-subtitle">{{ stock ? `${stock.basic.name} · ${stock.basic.symbol}` : t('stockDetail.loading') }}</p>
      </div>
    </div>
    ...
    <div v-if="stock" class="space-y-5 animate-fade-in">
      ...
      <div class="glass-card card-pad-lg">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          <div><span style="color:var(--text-disabled)">{{ t('sectors.industry') }}</span><div class="mt-0.5" style="color:var(--text-secondary)">{{ stock.basic.industry || '—' }}</div></div>
          <div><span style="color:var(--text-disabled)">{{ t('stockDetail.area') }}</span><div class="mt-0.5" style="color:var(--text-secondary)">{{ stock.basic.area || '—' }}</div></div>
          <div><span style="color:var(--text-disabled)">{{ t('stockDetail.market') }}</span><div class="mt-0.5" style="color:var(--text-secondary)">{{ stock.basic.market || '—' }}</div></div>
        </div>
      </div>

      <!-- Buffett + DCF -->
      <div v-if="stock.buffett" class="glass-card glow-cyan card-pad-lg">
        <div class="section-heading mb-4">{{ t('stockDetail.buffettAnalysis') }}</div>
        <div class="detail-metric-grid">
          <div class="detail-metric">
            <div class="detail-metric-label">{{ t('stockDetail.buffettComposite') }}</div>
            <div class="text-2xl font-bold font-mono" :style="{ color: scoreColor }">{{ stock.buffett.score?.toFixed(0) || '—' }}</div>
          </div>
          <div class="detail-metric">
            <div class="detail-metric-label">{{ t('stockDetail.roe') }}</div>
            <div class="text-lg font-mono mt-1" style="color:var(--text-secondary)">{{ fmtPctValue(stock.buffett.roe) }}</div>
          </div>
          <div class="detail-metric">
            <div class="detail-metric-label">{{ t('stockDetail.salesGrossMargin') }}</div>
            <div class="text-lg font-mono mt-1" style="color:var(--text-secondary)">{{ fmtPctValue(stock.buffett.gross_margin) }}</div>
          </div>
          <div class="detail-metric">
            <div class="detail-metric-label">{{ t('stockDetail.debtEquity') }}</div>
        ...
        <div v-if="stock.buffett.dcf_value" class="dcf-line">
          <span class="text-[10px]" style="color:var(--text-disabled)">{{ t('stockDetail.dcfIntrinsic') }} </span>
      ...
      <div v-if="stock.kline?.length" class="glass-card card-pad-lg">
        <div class="section-heading mb-4">{{ t('stockDetail.kline') }}</div>
        <div ref="chartRef" style="height:340px"></div>
      </div>

      <!-- Signals -->
      <div v-if="stock.signals" class="glass-card card-pad-lg">
        <div class="section-heading mb-4">{{ t('stocks.strategySignals') }}</div>
