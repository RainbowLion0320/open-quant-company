...
- When adding an OpenAI-compatible LLM provider, add it under `llm.providers.<provider>` with `protocol: openai_compatible`, `api_key_env`, `base_url`, `default_model`, optional `request.*`, and provider-specific `pricing.models`. Then point the relevant `llm.use_cases.*` entry, such as `agent_routing`, `agent_tool_planning`, `agent_response`, or `factor_hypothesis`, at that provider/model and verify `astroq config env --json` plus `astroq config validate --json`.
